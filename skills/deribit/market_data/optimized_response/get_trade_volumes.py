...
def optimize_get_trade_volume_response(json_data):
    ...
    for currency_data in trading_data:
        ...
        derivatives_ratio = (futures_vol + total_options) / spot_vol if spot_vol > 0 else float('inf')
        ...
        ai_ready_data["currencies"][currency_code] = {
            ...
            "distribution": {
                "spot": round(spot_vol / total_volume * 100, 1) if total_volume > 0 else 0,
                "futures": round(futures_vol / total_volume * 100, 1) if total_volume > 0 else 0,
                "options": {
                    "calls": round(calls_vol / total_volume * 100, 1) if total_volume > 0 else 0,
                    "puts": round(puts_vol / total_volume * 100, 1) if total_volume > 0 else 0,
                    "total": round((calls_vol + puts_vol) / total_volume * 100, 1) if total_volume > 0 else 0
                }
            },
            "insights": {
                "putCallRatio": round(put_call_ratio, 2) if put_call_ratio != float('inf') else None,
                "derivativesRatio": round(derivatives_ratio, 2) if derivatives_ratio != float('inf') else None,
                "dominantMarket": get_dominant_market(currency_data)
            }
        }
        
        # Add to market insights
        ai_ready_data["marketInsights"]["optionsSentiment"][currency_code] = interpret_put_call_ratio(put_call_ratio)
        ai_ready_data["marketInsights"]["volumeDistribution"][currency_code] = get_volume_distribution(currency_data)
        ai_ready_data["marketInsights"]["dominantMarkets"][currency_code] = get_dominant_market(currency_data)
    ...
    if any("_7d" in key or "_30d" in key for item in trading_data for key in item.keys()):
                ...
                if any(key.endswith("_7d") for key in currency_data.keys()):
                    ai_ready_data["currencies"][currency_code]["volumes_7d"] = {
                        ...
                        "options": {
                            ...
                            "total": currency_data.get("calls_volume_7d", 0) + currency_data.get("puts_volume_7d", 0)
                        }
                    }
                ...
                if any(key.endswith("_30d") for key in currency_data.keys()):
                    ai_ready_data["currencies"][currency_code]["volumes_30d"] = {
                        ...
                        "options": {
                            ...
                            "total": currency_data.get("calls_volume_30d", 0) + currency_data.get("puts_volume_30d", 0)
                        }
                    }
